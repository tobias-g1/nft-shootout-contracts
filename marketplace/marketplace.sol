...
contract NFTShootoutMarketplaceV1 is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
     ...
     * @param _SHOOAddress: SHOO address
    ...
    constructor(
        ...
        require(_SHOOAddress != address(0), "Operations: SHOO address cannot be zero");
    ...
    /**
     * @notice Buy token with SHOO by matching the price of an existing ask order
     ...
     */
    function buyTokenWithSHOO(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        IERC20(SHOO).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(SHOO).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != SHOO, "Operations: Cannot recover SHOO");
     ...
     * @param _withBNB: whether the token is bought with BNB (true) or SHOO (false)
     */
    function _buyToken(
        address _collection,
        uint256 _tokenId,
        ...
        // Transfer SHOO
        IERC20(SHOO).safeTransfer(askOrder.seller, netPrice);
